...
/** DOUBLE POROSITY INFINITE RESERVOIR TRANSIENT INTERPOROSITY FLOW SPHERES **/
/* References:
*
...
double ftsp(const double uD, const double omega, const double lambdas)
{
    ...
    fspheres = omega + (lambdas/(5.0*uD))
               * ( sqrt( 15.0*(1.0 - omega)*uD / lambdas )
                   / tanh( sqrt( 15.0*(1.0 - omega)*uD / lambdas ) )
                   - 1.0 );
